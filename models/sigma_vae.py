...
class SigmaVAE(nn.Module):
    def __init__(self, device='cuda', img_channels=3,model=None):
        super().__init__()
        self.device = device
        self.z_dim = 3
        ...
        demo_input = torch.ones([1,1,15])
        h_dim = self.encoder(demo_input).shape[2]
        print('h_dim', h_dim)
        ...
        if self.model == 'sigma_vae':
            ## Sigma VAE
            self.log_sigma = torch.nn.Parameter(torch.full((1,), 0)[0].float(), requires_grad=self.model == 'sigma_vae')

    @staticmethod
    def get_encoder(input_dim,int_dim):
